with manually_resolved_derived as (
  select distinct
    issue.entity_type,
    issue.entity_id,
    issue.issue_type,
...
open_duplicates as (
  select
    issue.id
  from public.issues as issue
  join manually_resolved_derived as resolved
    on resolved.entity_type = issue.entity_type
   and resolved.entity_id = issue.entity_id
   and resolved.issue_type = issue.issue_type
   and resolved.title = issue.title
  where issue.entity_type = 'placement'
...
update public.issues as issue
   set status = 'resolved',
       ...
       resolution_note = coalesce(
         ...
         'Auto-resolved because a matching derived placement issue was manually acknowledged and closed.'
       ),
       updated_at = now()
  from open_duplicates as duplicate_issue
  ...
  loop
    ...
    if severe_early_mortality_open then
      v_severe_desc :=
        ...
        (v_row.date_placed + 1)::text || ' through ' ||
        (v_row.date_placed + 7)::text || ' early-placement window.';
...
comment on function public.sync_derived_placement_issues(uuid[]) is
  'Auto-manages placement issues for severe early mortality and hatchery-quality incidents using app_settings thresholds and a master mortality_autowarn switch, while honoring manual resolves of derived issues.';
