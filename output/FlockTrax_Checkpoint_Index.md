...
- `2026-09-11` — Demo-to-production promotion ledger
  ...
  - living release-control list separating shared fixes that still require production promotion from demo-only work; currently tracks livehaul-adjusted dashboard population (strictly before today, with actual-over-target fallback), livehaul actual-head precedence, age-14 Starter-to-Grower ordering with preserved lifetime math, and the mobile Operations Calendar rolling 12-month history change, plus their demo commits and deployment/validation state
