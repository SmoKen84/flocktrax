...
type FeedProjectionReportTableProps = {
  ...
  densityDiagnostics: FeedProjectionDensityDiagnostic[];
};
            ...
            {rows.length > 0 ? (
              rows.map((row) => (
                <tr key={row.id}>
                  ...
                  <td>{row.source}</td>
                  <td>{row.bulkDensityLbPerFt3 === null ? row.weightBasis ?? "—" : `${row.bulkDensityLbPerFt3.toFixed(2)} lb/ft³ · ${row.weightBasis ?? ""}`}</td>
                  <td>{row.status}</td>
              ...
              <tr>
                <td className="feed-projection-report-empty" colSpan={13}>No feed is currently on order in this report scope.</td>
              </tr>
            )}
          </tbody>
        </table>
